...
class reseau2(object): #On classe par coordonnées
    def __init__(self):
        self.labels=columns_labels.generate(1)
        self.matrix=[[Cell(1,1,None)]]
        self.columnNumber=1
        self.rowNumber=1
    ...
    def addRow(self):
        ...
        for c in range(self.columnNumber):
            l.append(Cell(c+1,self.rowNumber+1,None))
    ...
    def addColumn(self):
        ...
        for r in range(self.rowNumber):
            self.matrix[r].append(Cell(self.columnNumber+1,r+1,None))
